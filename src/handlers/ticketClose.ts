...
export default async function ticketCloseFlow(message: Message) {
	...
	if (!status) {
		...
		await (await (await client.client.guilds.fetch(settings.GUILD_ID)).channels.fetch(settings.transcript_channel) as TextChannel).send({ files: [ staffTranscriptAttachment ], content: `Transcript from ModMail thread with user ${user} (<@${user}>)` })
